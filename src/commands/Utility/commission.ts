...
export default {
	...
	async execute(interaction: ChatInputCommandInteraction) {
		...
		const embed = new EmbedBuilder()
			...
			.setDescription(
				`You have a new commission from <@!${interaction.user.id}> for ${data?.name} ($${data?.price})!`,
			)
			.setColor('Green')
		...
		if (ownerUser) {
			...
			await interaction.reply({
				content:
					'I could not find the owner of this server. Please make sure they have DMs enabled.',
			})
		}
